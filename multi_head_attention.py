...
"""modified by Meng by adding point-wise convolution in paper 
...
class MultiHeadAttention(Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv1d = tf.keras.layers.Conv3D(filters=self._num_heads, kernel_size=1,data_format="channels_last")
    ...
    def _build_attention_(self, qkv_rank):
        """Builds multi-head dot-product attention computations.
    ...
    def _compute_attention_(                                                   ##original
    ...
    def _compute_attention(self,
        ...
        attention_scores=tf.transpose(attention_scores, perm=[0, 1, 3, 2])
        attention_scores = tf.expand_dims(attention_scores,axis=-1)
        # print('1')
        # print(attention_scores.shape)
        attention_scores = tf.transpose(attention_scores, perm=[0, 2, 3, 4,1]) ##channel last
        attention_scores = self.conv1d(attention_scores)
        # print('2')
        # print(attention_scores.shape)
        attention_scores = tf.transpose(attention_scores, perm=[0, 4, 1, 2, 3])  ##channel last back
